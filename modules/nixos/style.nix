...

{
  ...
  # ---- hyprpaper -----------------------------------------------------------
  # Deliberately NOT using home-manager's `services.hyprpaper` module (which
  # is what Stylix's own hyprpaper target drives): it hardcodes writing to
  # ~/.config/hypr/hyprpaper.conf, and that directory is a single
  # mkOutOfStoreSymlink to dotfiles/hypr/ (see home/jerzy/default.nix) — a
  # second Nix-managed entry landing inside it is what caused a
  # self-referential symlink and broke boot. Writing to an entirely separate
  # directory sidesteps that whole class of bug, at the cost of wiring it in
  # by hand: `exec-once` in dotfiles/hypr/hyprland.conf points hyprpaper at
  # this file with `-c`.
  home-manager.users.jerzy.xdg.configFile."hypr-stylix/hyprpaper.conf".text = ''
